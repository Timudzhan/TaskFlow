...
export { default } from './frontend/TaskFlow.jsx';
...
function AuthPage({ onAuth }) {
  ...
  function submit() {
    ...
    if (tab === "register") {
      ...
      if (users.find(u => u.email === email)) return setError("Email уже зарегистрирован.");
      const user = { id: Date.now(), name, email, pass };
      localStorage.setItem("tf_users", JSON.stringify([...users, user]));
      onAuth(user);
    } else {
